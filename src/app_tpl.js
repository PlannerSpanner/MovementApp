...
function primeSpeech(){try{
  ...
  dbg('prime: speak(" ", vol 0) called, returned '+r+', act='+dbgAct()+', speaking='+speechSynthesis.speaking+' pending='+speechSynthesis.pending+' paused='+speechSynthesis.paused);
...
function audioKeepAlive(on){
  ...
  try{
    ...
    if(!keepAlive){const a=document.createElement('audio');if(!a||typeof a.play!=='function')return;
      ...
      a.load();keepAlive=a;}
    const p=keepAlive.play();if(p&&p.then)p.then(()=>dbg('keepAlive play() resolved; src='+(keepAlive.currentSrc||'').slice(0,12)+' audioSession='+(navigator.audioSession?navigator.audioSession.type:'n/a')));if(p&&p.catch)p.catch(e=>{if(keepAliveOn&&keepAlive.paused)vlog('keep-alive media play rejected: '+(e&&e.message));});
...
function say(t){if(!voiceOn){dbg('say skipped (voice off): '+t);return;}try{
  ...
  dbg(tag+' speak() "'+t+'" voice='+(voicePick?voicePick.name:'default')+' act='+dbgAct()+' speaking='+speechSynthesis.speaking+' pending='+speechSynthesis.pending+' paused='+speechSynthesis.paused);
}catch(e){vlog('say threw: '+e.message);}}
// ---- on-device diagnostics: ?debug=1 renders a live panel on the page (no console on
// the phone). Off by default; every hook below is a no-op unless DEBUG is true. ----
const DEBUG=(()=>{try{return /[?&]debug=1/.test(location.search);}catch(e){return false;}})();
const dbgLog=[],dbgT0=Date.now();let dbgEl=null,dbgBody=null,dbgN=0;
const dbgAct=()=>{try{return navigator.userActivation?String(navigator.userActivation.isActive):'n/a';}catch(e){return '?';}};
function dbg(m){if(!DEBUG)return;dbgLog.push(((Date.now()-dbgT0)/1000).toFixed(1)+'s '+m);if(dbgLog.length>80)dbgLog.shift();dbgRender();}
function dbgStatus(){const L=[];try{
  L.push("'speechSynthesis' in window: "+('speechSynthesis' in window)+"  'SpeechSynthesisUtterance': "+('SpeechSynthesisUtterance' in window));
  if(window.speechSynthesis){const vs=speechSynthesis.getVoices()||[];
    L.push('getVoices().length: '+vs.length+(vs.length?'  first: '+vs.slice(0,4).map(v=>v.name+' ('+v.lang+(v.default?', default':'')+(v.localService?', local':'')+')').join(' | '):''));
    L.push('speaking: '+speechSynthesis.speaking+'  pending: '+speechSynthesis.pending+'  paused: '+speechSynthesis.paused+'  picked: '+(voicePick?voicePick.name:'none (system default)'));}
  L.push('AudioContext: '+(actx?actx.state:'not created')+'  audioSession: '+(navigator.audioSession?navigator.audioSession.type:'n/a')
    +'  keepAlive: '+(keepAlive?(keepAlive.paused?'paused':'playing')+' '+(keepAlive.currentSrc||'').slice(0,12):'none'));
...
function dbgRender(){if(!DEBUG)return;try{
  if(!dbgEl){if(!document.body)return;dbgEl=document.createElement('div');
    ...
    mk('Speak test',()=>{dbg('button: say() in gesture act='+dbgAct());say('Testing, one two three.');});
    mk('Speak raw default',()=>{const u=new SpeechSynthesisUtterance('Raw default voice test.');dbgWire(u,'raw');dbg('button: raw speak() act='+dbgAct());speechSynthesis.speak(u);});
    mk('Chime',()=>{dbg('button: chime');chime(660,.15,.6);});
    mk('cancel()+resume()',()=>{speechSynthesis.cancel();speechSynthesis.resume();dbg('button: cancel()+resume() done; speaking='+speechSynthesis.speaking+' paused='+speechSynthesis.paused);});
    mk('resume()',()=>{speechSynthesis.resume();dbg('button: resume(); paused='+speechSynthesis.paused);});
...
// wire an utterance's lifecycle events into the log (tag = 'prime' | 'cue#N' | 'raw')
function dbgWire(u,tag){if(!DEBUG)return;
  const t0=Date.now();const el=()=>'+'+((Date.now()-t0)/1000).toFixed(1)+'s';
  u.addEventListener('start',()=>dbg(tag+' START '+el()));
  u.addEventListener('end',()=>dbg(tag+' END '+el()+' (elapsed '+(u.elapsedTime|0)+')'));
  u.addEventListener('error',e=>dbg(tag+' ERROR '+el()+' code='+(e&&e.error)));
  u.addEventListener('pause',()=>dbg(tag+' pause event'));u.addEventListener('resume',()=>dbg(tag+' resume event'));}
if(DEBUG){dbg('debug view on; BUILDV '+(typeof BUILDV!=='undefined'?BUILDV:'(unstamped page)'));setInterval(dbgRender,1000);}
